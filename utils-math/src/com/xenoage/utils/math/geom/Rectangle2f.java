	...
	implements Shape
{
	...
	{
		return new Rectangle2f(position.x * f, position.y * f, size.width * f, size.height
			* f);
	}
	...
	{
		return new Rectangle2f(position.x * fx, position.y * fy, size.width * fx, size.height
			* fy);
	}
	...
	public Rectangle2f extend(Rectangle2f r)
	{
		...
		float newX2 = Math
			.max(this.position.x + this.size.width, r.position.x + r.size.width);
		float newY1 = Math.min(this.position.y, r.position.y);
		float newY2 = Math.max(this.position.y + this.size.height, r.position.y
			+ r.size.height);
	...
	{
		return new Rectangle2f((float) r.getMinX(), (float) r.getMinY(),
